...
class _SignUpPageState extends State<SignUpPage> {
  ...
  // ============================================================
  // SMART EMAIL DETECTION WITH STUDENT/ALUMNI AUTO-DETECTION
  // ============================================================
  String detectRoleFromEmail(String email) {
    if (!isMIUEmail(email)) return "Student";
    ...
    if (hasNumbers) {
      // Extract registration year from email
      // Example: mostafa2201035 -> "22" means joined in 2022
      ...
      if (yearMatch != null) {
        ...
        if (registrationYear != null) {
          ...
          // If joined 4+ years ago, they're Alumni
          // If joined less than 4 years ago, they're Student
      ...
      // If we can't parse the year, default to Student
      return "Student";
    } else {
      // Instructor/TA email (only names, no numbers)
      return "Instructor";
    }
  }

  // ============================================================
  // CALCULATE ACADEMIC YEAR FROM EMAIL
  // ============================================================
  int calculateAcademicYear(String email) {
    if (!isMIUEmail(email)) return 1;
    ...
    if (yearMatch != null) {
      ...
      if (registrationYear != null) {
        ...
        // Calculate which year they're in (1-4)
        if (yearsSinceJoining >= 4) {
          return 4; // Alumni (graduated, show as Year 4)
        } else if (yearsSinceJoining == 3) {
          return 4; // Year 4 (final year)
        } else if (yearsSinceJoining == 2) {
          return 3; // Year 3
        } else if (yearsSinceJoining == 1) {
          return 2; // Year 2
        } else {
          return 1; // Year 1 (freshman)
        }
      }
    }
  ...
  void _onEmailChanged(String email) {
    if (isMIUEmail(email)) {
      final detectedRole = detectRoleFromEmail(email);
      final calculatedYear = calculateAcademicYear(email);
      
      setState(() {
        _selectedRole = detectedRole;
        _selectedAcademicYear = calculatedYear;
        _showAcademicYear = (detectedRole == "Student" || detectedRole == "Alumni");
      });

      // Show info message
      if (detectedRole == "Alumni") {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("🎓 Welcome Alumni! Your account will be marked as graduated."),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("📚 Detected as Year $_selectedAcademicYear student"),
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                // ACADEMIC YEAR - SHOW FOR STUDENTS AND ALUMNI
                ...
                // ROLE DROPDOWN - AUTO-DETECTED BUT EDITABLE
                DropdownButtonFormField<String>(
                  value: _selectedRole,
                  decoration: InputDecoration(
                    labelText: "Role",
                    prefixIcon: Icon(
                      _selectedRole == "Alumni" ? Icons.school : Icons.badge, 
                      color: Colors.red,
                    ),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                    helperText: "✓ Auto-detected from email",
                    helperStyle: TextStyle(
                      color: _selectedRole == "Alumni" ? Colors.purple : Colors.green[700], 
                      fontSize: 11,
                      fontWeight: FontWeight.bold,
